...
### EXTRACT BASE SYSTEM
```sh
sudo tar xvf rootfs_amd64.tar.xz -C $ROOTFS_DIR
```
...
- **INSTALL SYSTEMD**
    ...
    - **Network**
        ...
        - **ENP**
            ```bash
            cat << EOF > /etc/systemd/network/20-enp.network
            [Match]
            Name=*enp*
        ...
        - **ENX**
            ```bash
            cat << EOF > /etc/systemd/network/20-enx.network
            [Match]
            Name=*enx*
        ...
        - **wlp**
            ```bash
            cat << EOF > /etc/systemd/network/20-wlp.network
            [Match]
            Name=*wlp*
